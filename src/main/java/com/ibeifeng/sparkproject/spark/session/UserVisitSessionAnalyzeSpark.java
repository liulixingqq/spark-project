...
//		sc.checkpointFile("hdfs://");
		...
		// 生成模拟测试数据  dataframe 临时注册几张表
		SparkUtils.mockData(sc, sqlContext);  
		
		// 创建需要使用的DAO组件
		ITaskDAO taskDAO = DAOFactory.getTaskDAO();
		
		// 首先得查询出来指定的任务，并获取任务的查询参数 本地测试 taskId 2
...
//		sessionid2actionRDD.checkpoint();
		...
		System.out.println("过滤之前 "+sessionid2actionRDD.count());
		...
		JavaPairRDD<String, String> extractSessionidsRDD = time2sessionsRDD.flatMapToPair(
				...
				new PairFlatMapFunction<Tuple2<String,Iterable<String>>, String, String>() {
					...
					public Iterable<Tuple2<String, String>> call(
						...
						int index = 0;
						// 举例子 每个小时总共10个session 需要抽取 3个session
						//   extractIndexList 有3 个session  time2sessionsRDD 有10个session
						while(iterator.hasNext()) {
